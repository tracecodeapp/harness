...
| `assemblies-02.pack` | 2,387,070 | 42 | `b4ce85d14a5deadf16cef1291f42b148b288c391c017eb507ad6dae3664f2cfd` |
| `assemblies-03.pack` | 2,383,417 | 41 | `7cfb755d793712bc94d4e24e6c19a2c8829059b38346e3ee32c7059134cd6fdc` |

The runner changed from 95 files / 13,122,947 raw bytes / 4,313,090
Brotli-6 bytes to 12 files / 12,814,019 raw bytes / 4,081,616 Brotli-6
bytes. Its final content-tree SHA-256 is
`c5d65c14e9d2a20e6a3a516db04fe2c6226ef9bc1a12a8afc3fd9a314af64981`.
...
## Reproducible source-control role artifacts

The runtime packs solve browser request fan-out; a separate build-time layer
keeps generated publish trees out of source review:

| Role | Canonical ZIP SHA-256 | ZIP bytes | Expanded files/bytes | Tree SHA-256 |
|---|---|---:|---:|---|
| Compiler | `88a5186a62399e703d61a43036cb6eb7f3cba946632ac2bb0db1648a49b482e3` | 18,073,336 | 241 / 50,213,955 | `e532fb2e3e581a01fe33de87d0fa480188b5f6ee8d7e62b98b5d7446b6d334cc` |
| Runner | `29d82020d99792a757b39a3eaa287c13a6d0158149ebad76a3311a09b7b7688d` | 4,670,430 | 12 / 12,813,595 | `926dc991c05a06f896fbcff4fe72c63be31af8a835a479bbb134cb89c8e02540` |

`pnpm update:csharp-runtime` regenerates them with the exact SDK in
`runtimes/csharp/Directory.Build.props`; `pnpm materialize:csharp-role-assets`
validates archive and tree integrity before an atomic swap. Package sync and
browser tests invoke it automatically. Tests cover byte-exact regeneration,
clean-checkout materialization, tamper repair, corrupt/stale rejection, and
npm exclusion of build-time ZIPs.

Git revision metadata is omitted from informational versions so committing
cannot invalidate generated bytes. Emscripten C source paths and generated JS
include paths are also mapped to canonical `/tracecode/source` and
`/tracecode/dotnet` roots, and the build fails if the checkout or SDK root
survives in any role asset. Forced `SourceRevisionId` variants produced
identical archives (`source-revision-repro.CvRjW8/`). Builds with the SDK at
both the worktree-local `.dotnet/csharp-wasm` path and the space-containing
`/Volumes/External Storage/.../dotnet-sdk-10.0.110` path produced the exact
compiler and runner hashes in the table above. The deployed trees are
byte-identical and do not alter performance, Mux, or TraceKernel boundaries.
A fresh Chromium corpus matched all 200/200 prior production rows:

- candidate `e477978a16382ae252beee9d7f2ae054470e00d5a4d460e5b6f9b5e5627eaa78`;
...
24,283 additions to 101 files / 10,461 additions. The remaining generated
...
The post-review pass made ten boundary corrections without weakening the
...
- prepared and general host attempts reset input and trace context in their
  outer lifecycle boundary, including failures before assembly loading;
- native source/debug paths and generated JS include comments are
  installation-root-independent, with build-time leakage checks and a
  cross-root byte-identity regression.

The full browser worker suite retained the sub-second compilation result:
668.97 ms cold compile, 1,104 ms cold end-to-end, 22.77 ms edited compile,
...
Finally, the complete Chromium production Judge -> TraceKernel corpus on the
path-normalized artifacts matched the preceding hardened baseline exactly for
all 200/200 rows and every compared receipt field. Browser wall time was
127,397.24 ms, observed process peak was 1,253,179,392 bytes, and post-context
settled RSS was 197,623,808 bytes. Raw evidence:

- `production-integration/path-normalized-review-final-production-corpus-chromium-200.json`
