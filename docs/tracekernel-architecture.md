# TraceKernel architecture

Status: initial implementation contract

## Purpose

TraceKernel is the authoritative owner of processes, descriptors, blocking
I/O, shared files, runtime leases, and the foundation of the session-local
network.
...
The transport keeps three layers separate:
...
Browser JavaScript is the first product runtime attached to the syscall wire
contract. TypeScript uses the same adapter after compilation; the compiler
...
legacy runner pipe. Shell presentation and the product journal remain
transitional. Public PID 0 host operations are physically backed by an
invisible, protected TraceKernel service process, so host HTTP listeners and
direct host descriptor syscalls use the same extracted session descriptor
tables, TCP namespace, teardown, and limits as runtime processes. Runtime
structured HTTP uses that same port namespace as raw runtime sockets.
...
The service process is hidden from unprivileged process enumeration but remains
visible to trusted kernel diagnostics; PID 0 stays the stable public attribution
identity rather than becoming a descriptor bypass.
...
| Runtime API surface | TraceKernel syscall |
| --- | --- |
...
process-control presentation, and journal/resource event attribution. Host
services now have an invisible protected process in the extracted session;
