# TraceKernel kernelization contract

## Supported contract

TraceKernel is the authoritative browser-native kernel. Its supported
architecture is:
...
- Package build and packed-file inspection expose only the documented root
  entry and intended artifacts.
- Generated worker and runtime assets match their checked-in sources.

## Executable kernelization profiles

The kernelization criteria are executable rather than a prose-only checklist:

```bash
# Bounded local kernel and adapter validation.
pnpm test:tracekernel:kernelization:bounded

# Full local validation before a kernel-affecting release. Runs the extended
# soak, every real browser adapter, the three-engine provider/live-TKFS
# matrices, TraceJVM's independent suite, and packed artifact gates.
TRACECODE_TRACEJVM_ROOT=/absolute/path/to/tracejvm \
  pnpm test:tracekernel:kernelization:full

# Independently rerunnable surfaces.
pnpm test:tracekernel:kernelization:tracejvm
pnpm test:tracekernel:kernelization:artifacts
```

The bounded profile runs:
...
The kernelization gates fail if any of these conditions are observed:
...
Physical iPad validation remains a kernelization sign-off because it cannot be
represented truthfully by desktop CI. WebKit iPad emulation belongs to
TraceJVM's independent standalone suite, but it is not recorded as a
...
```bash
TRACECODE_TRACEJVM_ROOT=/absolute/path/to/tracejvm \
  pnpm test:tracekernel:physical:check
```
...
```bash
TRACECODE_TRACEJVM_ROOT=/absolute/path/to/tracejvm \
  pnpm test:tracekernel:physical -- --tunnel
```
...
`reports/tracekernel-physical/` directory. A physical kernelization sign-off
requires two passing reports from the same device, with the second produced
after the background/foreground transition. Kernel-host lifecycle validation
also backgrounds Safari while a run is still active and confirms that the
original in-memory session resumes without filesystem, descriptor, socket,
process, or worker-isolation loss.
...
TraceKernel kernelization profiles, and physical-device validation run locally.
They must not be added to automatic or scheduled GitHub workflows.

The local generic provider matrix can continue to monitor CheerpJ for legacy
compatibility. It is intentionally not a TraceKernel kernelization dependency:
Java is gated by the independent TraceJVM and integration suites above.
