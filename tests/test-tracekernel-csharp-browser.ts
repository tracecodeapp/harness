...
async function main(): Promise<void> {
  const tempRoot = await mkdtemp(join(tmpdir(), 'tracekernel-csharp-'));
  ...
  try {
    ...
    try {
          ...
          files: [
            ...
            },
            {
              ...
              contents: [
                ...
                'int resizeSignals = 0;',
                'TraceKernel.KernelTerminal.WindowSizeChanged += () => resizeSignals++;',
                ...
                '}',
                'var initialPipePoll = TraceKernel.KernelPoll.Wait(',
                '    new[]',
                '    {',
                '        new TraceKernel.KernelPollRequest(nonblockingPipe.ReadEnd.Number, TraceKernel.KernelPollEvents.Read),',
                '        new TraceKernel.KernelPollRequest(nonblockingPipe.WriteEnd.Number, TraceKernel.KernelPollEvents.Write),',
                '    },',
                '    timeoutMs: 0',
                ');',
                'bool initialPipeReadiness = initialPipePoll.Count == 1',
                '    && initialPipePoll[0].Descriptor == nonblockingPipe.WriteEnd.Number',
                '    && initialPipePoll[0].Events.HasFlag(TraceKernel.KernelPollEvents.Write);',
                ...
                'var readablePipePoll = TraceKernel.KernelPoll.Wait(',
                '    new[] { new TraceKernel.KernelPollRequest(nonblockingPipe.ReadEnd.Number, TraceKernel.KernelPollEvents.Read) },',
                '    timeoutMs: 1000',
                ');',
                'bool finalPipeReadiness = readablePipePoll.Count == 1',
                '    && readablePipePoll[0].Events.HasFlag(TraceKernel.KernelPollEvents.Read)',
                '    && readablePipePoll[0].Events.HasFlag(TraceKernel.KernelPollEvents.Hangup);',
                ...
                '    && initialPipeReadiness && finalPipeReadiness',
                ...
                '    }',
                ');',
                'bool nonblockingWait = !TraceKernel.KernelProcess.TryWaitChild(-1, out _)',
                '    && !groupLeader.TryWait(out _);',
                ...
                'var groupLeaderIdentity = TraceKernel.KernelProcess.GetIdentity(groupLeader.Pid);',
                'bool groupIdentity = int.Parse(groupPids[0]) == groupLeader.Pid',
                '    && int.Parse(groupPids[1]) != groupLeader.Pid',
                '    && groupLeaderIdentity.ProcessGroupId == groupLeader.Pid',
                '    && groupLeaderIdentity.SessionId == groupLeader.Pid;',
                'TraceKernel.KernelProcess.SignalProcessGroup(',
                '    groupLeader.Pid,',
                '    TraceKernel.KernelSignal.Quit',
                ');',
                'var groupExit = TraceKernel.KernelProcess.WaitChild(-groupLeader.Pid);',
                'var localWaitChild = TraceKernel.KernelProcess.Start(',
                '    "javascript",',
                '    "node",',
                '    new[] { "wait-child.js" }',
                ');',
                'var localWaitExit = TraceKernel.KernelProcess.WaitChild(0);',
                'bool noChildrenRemain = false;',
                'try',
                '{',
                '    TraceKernel.KernelProcess.TryWaitChild(-1, out _);',
                '}',
                'catch (TraceKernel.TraceKernelException error) when (error.Code == "ECHILD")',
                '{',
                '    noChildrenRemain = true;',
                '}',
                'System.Threading.Thread.Sleep(650);',
                'bool processGroupKilled = nonblockingWait && groupIdentity',
                '    && groupExit.Pid == groupLeader.Pid',
                '    && groupExit.Signal == TraceKernel.KernelSignal.Quit',
                '    && localWaitExit.Pid == localWaitChild.Pid',
                '    && localWaitExit.ExitCode == 42',
                '    && noChildrenRemain',
                ...
                'javascriptListener.Nonblocking = true;',
                'bool nonblockingAccept = false;',
                'try',
                '{',
                '    using var unexpected = javascriptListener.Accept();',
                '}',
                'catch (TraceKernel.TraceKernelException error) when (error.Code == "EAGAIN")',
                '{',
                '    nonblockingAccept = true;',
                '}',
                'javascriptListener.Nonblocking = false;',
                ...
                '    }',
                ');',
                'bool javascriptListenerReady = TraceKernel.KernelPoll.Wait(',
                '    new[] { new TraceKernel.KernelPollRequest(javascriptListener.Descriptor, TraceKernel.KernelPollEvents.Read) },',
                '    timeoutMs: 5000',
                ').Any(result => result.Events.HasFlag(TraceKernel.KernelPollEvents.Read));',
                'using var javascriptConnection = javascriptListener.Accept();',
                'bool javascriptStreamReady = TraceKernel.KernelPoll.Wait(',
                '    new[] { new TraceKernel.KernelPollRequest(javascriptConnection.Descriptor, TraceKernel.KernelPollEvents.Read) },',
                '    timeoutMs: 5000',
                ').Any(result => result.Events.HasFlag(TraceKernel.KernelPollEvents.Read));',
                'using var pendingSocket = TraceKernel.KernelSocket.Create();',
                'pendingSocket.Nonblocking = true;',
                'bool connectInProgress = false;',
                'try',
                '{',
                '    pendingSocket.Connect(javascriptEndpoint.Host, javascriptEndpoint.Port);',
                '}',
                'catch (TraceKernel.TraceKernelException error) when (error.Code == "EINPROGRESS")',
                '{',
                '    connectInProgress = true;',
                '}',
                'bool pendingSocketReady = TraceKernel.KernelPoll.Wait(',
                '    new[] { new TraceKernel.KernelPollRequest(pendingSocket.Descriptor, TraceKernel.KernelPollEvents.Write) },',
                '    timeoutMs: 5000',
                ').Any(result => result.Events.HasFlag(TraceKernel.KernelPollEvents.Write));',
                'bool pendingSocketClean = pendingSocket.GetAndClearConnectError() == null;',
                'using (var pendingConnection = javascriptListener.Accept()) { }',
                ...
                '    && processGroupKilled',
                '    && nonblockingAccept',
                '    && javascriptListenerReady',
                '    && javascriptStreamReady',
                '    && connectInProgress',
                '    && pendingSocketReady',
                '    && pendingSocketClean',
      ...
      console.log(JSON.stringify({
        schema: 'tracekernel-csharp-conformance-v1',
        ...
        managedDescriptorPolling: true,
        managedTcpSockets: true,
        managedNonblockingTcpSockets: true,
