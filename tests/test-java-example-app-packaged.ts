...
async function createPackagedExampleApp(tempRoot: string): Promise<string> {
  ...
  const packageJson = {
    name: '@tracecode/harness-example-web-ide-java-packaged-smoke',
    private: true,
    type: 'module',
    scripts: {
...
async function runWithTempRoot(tempRoot: string): Promise<void> {
  ...
  const previewPort = 4800 + Math.floor(Math.random() * 200);
...
test('java example app packaged', main);
