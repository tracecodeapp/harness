...
import type { JavaConformanceFixture, JavaConformanceRunResult, JavaExecutionResult } from '../tests/conformance/java-native-runner';
import { runJavaNativeFixture } from '../tests/conformance/java-native-runner';
...
interface RejectedFixture {
  ...
  run?: JavaConformanceRunResult;
}

function usage(): string {
  return [
    'Usage: tsx scripts/import-java-conformance-fixtures.ts --input fixtures.json [options]',
    '',
    'Options:',
...
function validateCandidate(value: unknown): string[] {
  ...
  if (typeof value.id === 'string' && !/^java_[a-z0-9_]+$/.test(value.id)) {
    errors.push('id must match /^java_[a-z0-9_]+$/');
  }
...
function normalizeCandidate(value: Record<string, unknown>): JavaConformanceFixture {
  const fixture: JavaConformanceFixture = {
...
async function readExistingFixtures(outputPath: string): Promise<JavaConformanceFixture[]> {
  try {
    const value = JSON.parse(await readFile(outputPath, 'utf8'));
    return Array.isArray(value) ? value as JavaConformanceFixture[] : [];
  } catch (error) {
...
function mergeFixtures(existing: JavaConformanceFixture[], incoming: JavaConformanceFixture[]): JavaConformanceFixture[] {
  const byId = new Map<string, JavaConformanceFixture>();
...
function summarizeExecutionResult(result: JavaExecutionResult | undefined): unknown {
...
async function writeSummary(args: ImportArgs, accepted: JavaConformanceFixture[], rejected: RejectedFixture[]): Promise<void> {
  await mkdir(args.failureDir, { recursive: true });
  const summary = {
    language: LANGUAGE,
    runner: 'native-javac-reflection',
...
const accepted: JavaConformanceFixture[] = [];
...
for (const [index, candidate] of candidates.entries()) {
  ...
  console.log(`RUN: java conformance import ${fixture.id}`);
  const run = await runJavaNativeFixture(fixture);
  if (run.success) {
...
console.log(
  `java conformance import complete: accepted=${accepted.length}, rejected=${rejected.length}` +
