...
import type { JavaScriptConformanceFixture, JavaScriptConformanceRunResult, JavaScriptExecutionResult } from '../tests/conformance/javascript-runner';
import { createInitializedJavaScriptConformanceBridge, runJavaScriptConformanceFixture } from '../tests/conformance/javascript-runner';
...
interface RejectedFixture {
  ...
  run?: JavaScriptConformanceRunResult;
}

function usage(): string {
  return [
    'Usage: tsx scripts/import-javascript-conformance-fixtures.ts --language javascript|typescript --input fixtures.json [options]',
    '',
    'Options:',
...
function positionalArgs(argv: string[]): string[] {
  const optionsWithValues = new Set(['--input', '--language', '--out', '--failure-dir', '--limit']);
...
function parseArgs(argv: string[]): ImportArgs {
  ...
  const language = readOption(argv, 'language');
...
function validateCandidate(value: unknown, language: 'javascript' | 'typescript'): string[] {
  const errors: string[] = [];
  ...
  if (typeof value.id === 'string' && !new RegExp(`^${language}_[a-z0-9_]+$`).test(value.id)) {
    errors.push(`id must match /^${language}_[a-z0-9_]+$/`);
  }
  ...
  if (typeof value.source === 'string' && /\brequire\s*\(\s*["']fs["']\s*\)/.test(value.source) && value.entryStyle !== 'stdin_program') {
    errors.push('fs-based programs must use entryStyle stdin_program');
  }
  return errors;
}

function normalizeCandidate(value: Record<string, unknown>): JavaScriptConformanceFixture {
  const fixture: JavaScriptConformanceFixture = {
...
async function readExistingFixtures(outputPath: string): Promise<JavaScriptConformanceFixture[]> {
  try {
    const value = JSON.parse(await readFile(outputPath, 'utf8'));
    return Array.isArray(value) ? value as JavaScriptConformanceFixture[] : [];
  } catch (error) {
...
function mergeFixtures(existing: JavaScriptConformanceFixture[], incoming: JavaScriptConformanceFixture[]): JavaScriptConformanceFixture[] {
  const byId = new Map<string, JavaScriptConformanceFixture>();
...
function summarizeExecutionResult(result: JavaScriptExecutionResult | undefined): unknown {
    ...
    ...(trace
      ? {
          trace: {
            ...
            lineEventCount: trace.lineEventCount ?? result.lineEventCount,
            traceStepCount: trace.traceStepCount ?? result.traceStepCount,
...
async function writeSummary(args: ImportArgs, accepted: JavaScriptConformanceFixture[], rejected: RejectedFixture[]): Promise<void> {
  await mkdir(args.failureDir, { recursive: true });
  const summary = {
...
const bridge = await createInitializedJavaScriptConformanceBridge(args.language);
const accepted: JavaScriptConformanceFixture[] = [];
...
for (const [index, candidate] of candidates.entries()) {
  ...
  const validationErrors = validateCandidate(candidate, args.language);
  ...
  console.log(`RUN: ${args.language} conformance import ${fixture.id}`);
  const run = await runJavaScriptConformanceFixture(bridge, fixture, args.language);
  if (run.success) {
...
console.log(
  `${args.language} conformance import complete: accepted=${accepted.length}, rejected=${rejected.length}` +
