...
const traceJVMRoot = resolve(
  argumentValue('--tracejvm-root') ??
    ...
    join(repositoryRoot, '../tracejvm-tracekernel-013')
);
...
if (!existsSync(traceJVMWorker) || !existsSync(traceJVMRuntime)) {
  throw new Error(
    `TraceJVM release assets are missing beneath ${traceJVMRoot}. ` +
...
const temporaryDirectory = mkdtempSync(
  join(tmpdir(), 'tracekernel-013-physical-')
);
...
const reportDirectory = resolve(
  argumentValue('--report-dir') ??
    join(repositoryRoot, 'reports/tracekernel-013-physical')
);
...
await build({
  entryPoints: [
    resolve(
      repositoryRoot,
      'tests/fixtures/tracekernel-013-physical-entry.ts'
    ),
  ],
  bundle: true,
  format: 'iife',
...
const physicalHtml = readFileSync(
  resolve(
    repositoryRoot,
    'tests/fixtures/tracekernel-013-physical.html'
  )
);
...
async function runAutomatedCheck(engine) {
  ...
  try {
    ...
    console.log(
      `PASS: TraceKernel 0.13 physical runner in ${engine} ` +
