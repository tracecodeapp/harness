...
import type { CSharpConformanceFixture, CSharpConformanceRunResult, CSharpExecutionResult } from '../tests/conformance/csharp-browser-runner';
import { createCSharpConformanceBridge, runCSharpConformanceFixture } from '../tests/conformance/csharp-browser-runner';
...
interface RejectedFixture {
  ...
  run?: CSharpConformanceRunResult;
}

function usage(): string {
  return [
    'Usage: tsx scripts/import-csharp-conformance-fixtures.ts --input fixtures.json [options]',
    '',
    'Options:',
...
function validateCandidate(value: unknown): string[] {
  ...
  if (typeof value.id === 'string' && !/^csharp_[a-z0-9_]+$/.test(value.id)) {
    errors.push('id must match /^csharp_[a-z0-9_]+$/');
  }
...
function normalizeCandidate(value: Record<string, unknown>): CSharpConformanceFixture {
  const fixture: CSharpConformanceFixture = {
...
async function readExistingFixtures(outputPath: string): Promise<CSharpConformanceFixture[]> {
  try {
    const value = JSON.parse(await readFile(outputPath, 'utf8'));
    return Array.isArray(value) ? value as CSharpConformanceFixture[] : [];
  } catch (error) {
...
function mergeFixtures(existing: CSharpConformanceFixture[], incoming: CSharpConformanceFixture[]): CSharpConformanceFixture[] {
  const byId = new Map<string, CSharpConformanceFixture>();
...
function summarizeExecutionResult(result: CSharpExecutionResult | undefined): unknown {
  if (!result) return undefined;
  const { trace, events, ...rest } = result;
  return {
    ...rest,
    eventCount: Array.isArray(events) ? events.length : undefined,
    trace: trace && Array.isArray(trace.events) ? { eventCount: trace.events.length, firstEvents: trace.events.slice(0, 20) } : undefined,
  };
}
...
async function writeSummary(args: ImportArgs, accepted: CSharpConformanceFixture[], rejected: RejectedFixture[]): Promise<void> {
  await mkdir(args.failureDir, { recursive: true });
  const summary = {
    language: LANGUAGE,
    runner: 'browser-wasm-worker',
...
const accepted: CSharpConformanceFixture[] = [];
const rejected: RejectedFixture[] = [];
const bridge = await createCSharpConformanceBridge();
...
try {
  for (const [index, candidate] of candidates.entries()) {
    ...
    console.log(`RUN: csharp conformance import ${fixture.id}`);
    const run = await runCSharpConformanceFixture(bridge, fixture);
    if (run.success) {
...
console.log(
  `csharp conformance import complete: accepted=${accepted.length}, rejected=${rejected.length}` +
