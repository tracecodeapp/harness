...
function augmentJavaProjectFileMutations(source) {
  return String(source ?? '')
    .replace(/\bjava\.nio\.file\.Files\.(readString|readAllBytes|readAllLines|lines|list|newDirectoryStream|newInputStream|newBufferedReader|exists|notExists|isDirectory|isRegularFile|isReadable|isWritable|size|writeString|write|createFile|createDirectory|createDirectories|createTempFile|createTempDirectory|newOutputStream|newBufferedWriter|newByteChannel|deleteIfExists|delete|copy|move)\s*\(/g, 'tracecode.browser.ProjectEvents.$1(')
